...
export const monitorSchedulerTicks = sqliteTable(
  "monitor_scheduler_ticks",
  {
    ...
    triggerSource: text("trigger_source").notNull().default("cloudflare_cron"),
    leaseToken: text("lease_token"),
    leaseExpiresAt: text("lease_expires_at"),
    recoveredJobCount: integer("recovered_job_count").notNull().default(0),
    previousTickAt: text("previous_tick_at"),
...
export const researchTracks = sqliteTable(
  "research_tracks",
  {
    ...
    buildAttemptCount: integer("build_attempt_count").notNull().default(0),
    buildSourceStatusJson: text("build_source_status_json").notNull().default("[]"),
