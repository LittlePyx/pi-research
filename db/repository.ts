...
async function bootstrapSchema(database: D1Database) {
  await database.batch([
    ...
    database.prepare("CREATE TABLE IF NOT EXISTS paper_abstract_recovery (paper_id TEXT PRIMARY KEY REFERENCES monitored_papers(id) ON DELETE CASCADE, space_id TEXT NOT NULL REFERENCES research_spaces(id) ON DELETE CASCADE, status TEXT NOT NULL DEFAULT 'pending', source_url TEXT NOT NULL DEFAULT '', attempted_json TEXT NOT NULL DEFAULT '[]', retry_at INTEGER NOT NULL DEFAULT 0, lease_until INTEGER NOT NULL DEFAULT 0, lock_token TEXT, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)"),
