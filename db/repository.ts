...
export async function ensureSchema(database = getDatabase()) {
  await database.batch([
    ...
    database.prepare("CREATE TABLE IF NOT EXISTS monitor_scan_jobs (id TEXT PRIMARY KEY NOT NULL, space_id TEXT NOT NULL REFERENCES research_spaces(id) ON DELETE CASCADE, status TEXT NOT NULL DEFAULT 'queued', current_horizon TEXT NOT NULL DEFAULT '', current_source TEXT NOT NULL DEFAULT '', progress INTEGER NOT NULL DEFAULT 0, discovered_count INTEGER NOT NULL DEFAULT 0, reviewed_count INTEGER NOT NULL DEFAULT 0, recommended_count INTEGER NOT NULL DEFAULT 0, attempt INTEGER NOT NULL DEFAULT 1, error TEXT, started_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, completed_at TEXT, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)"),
    database.prepare("CREATE INDEX IF NOT EXISTS idx_monitor_scan_jobs_space_updated ON monitor_scan_jobs(space_id, updated_at)"),
    database.prepare("CREATE TABLE IF NOT EXISTS monitor_discovery_coverage (id TEXT PRIMARY KEY NOT NULL, space_id TEXT NOT NULL REFERENCES research_spaces(id) ON DELETE CASCADE, horizon TEXT NOT NULL, source_key TEXT NOT NULL, channel TEXT NOT NULL, query_key TEXT NOT NULL, next_cursor INTEGER NOT NULL DEFAULT 0, attempt_count INTEGER NOT NULL DEFAULT 0, candidate_count INTEGER NOT NULL DEFAULT 0, new_candidate_count INTEGER NOT NULL DEFAULT 0, last_scanned_at TEXT, last_error TEXT, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)"),
