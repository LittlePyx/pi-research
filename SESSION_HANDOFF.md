...
- 已进一步确定并复现身份破坏缺陷：原14篇第14篇canonicalId是doi:10.1130/0091-7613(1990)018<0812:lbotao>2.3.co;2。旧quickScreenBatch用HTML正文清理cleanText处理身份，把<0812:lbotao>删成空格；即使模型原样返回也找不到。对真实v208函数喂同一份完美14篇模拟响应，旧代码两次调用仍0保存；v209函数一次返回14且保留原DOI。v209身份匹配只整理空白，不做HTML清理，并保留有效部分结果。03:12不是发布时间：实际04:12:15 UTC v209成功，源码00c2bc8a695e06330ad0f9565046470a2ed15888，部署appgdep_6aa22e1465a08191a7ea1e2550a22bfa，CI34436191627通过。当前本地只补真实DOI回归，产品代码不变，完整567项通过；等待原任务自然生产进度，不能把本地14/14当生产59/59。
- 最新突破是根因取证，不是扫描已恢复：04:03自然调度接续9e4bab7f，14篇首请求14.583秒HTTP200/stop、1333输出token，却在validate因至少一个canonicalId未匹配而丢弃整批；随后仍14篇重试才12秒headers超时。v207只对首错timeout缩批，故未解决此身份错误。14篇实际输入已完整复原到outputs/screening-9e4bab7f-exact-inputs.json，原计划有trace与证据。当前落实保留唯一身份且逐项校验通过的fast部分结果、未匹配留队列；rescue仍要求全批，具体生产越过45/59待发布后核对。
- 用户随后明确要求优先解决45/59，并同意先取得剩余14篇输入与分请求耗时再修复。当前在codex/screening-request-diagnostics，基于main@b7b6471隔离实现，不把草稿PR#1的引用UI一起发布。为原fast/rescue筛选增加带scanJobId/traceId的请求诊断：响应头/响应体/解析/校验/用量阶段、HTTP状态、耗时、结束原因和计数；失败时记录确切书目/900字摘要输入的有界片段，超长明确标记截断。不记录凭据、请求头、用户记忆、模型原文或原始错误消息。复用可靠性事件表并输出结构化Worker日志，无迁移、模型参数/24与12秒超时/重试/质量门槛变化；发布与自然取证结果继续记原计划。
- 诊断源码d5376aa90261c6d7b58faf5f527c500862c0d3cb已快进main并同步GitHub及Sites独立绑定仓库；CI34434906703通过。v208于03:54:22 UTC部署成功，版本appgprj_6a83f86ecca081919f3094b285bc2b1d~appgver_2465bd19cd588191ade2c8bea13adb12，部署appgdep_6aa229e113b48191826d28a5225a72a0，仍public/环境修订6。发布前562项通过；随后仅增加本机HTTP200空行保活/响应体超时测试，未变产品代码的完整563项通过。当前实际任务为9e4bab7f-285d-4327-a975-e4e15b24289c、attempt14、父96767，03:35上轮页面访问触发，仍45/59；不能继续只看96767。新诊断自然样本尚待读取，不宣称已修复。
