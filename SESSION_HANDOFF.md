...
## 当前优先：今日/论文库慢加载与路线视觉层级（实现完成，待发布验证）
...
- 当前最后完整检查 602 通过，随后少量 CSS/加载文案修改还需最终构建。临时预览 8114/session 87500、QA tab10/生产 tab9 结束需清理，viewport 已 reset。发布包上传前轮超时，本轮需带新源码重新打包并尝试；不能直接发布旧 66b23a7 包覆盖本轮修复。用户 docs/ 保留。
