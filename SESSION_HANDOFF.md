...
## 2026-09-16：切换 DeepSeek V4.1 Flash（发布准备）
...
- 构建及626测试通过。真实隔离 Worker/D1 的图谱两次模型请求断言 deepseek-flash；旧 Pro 学习路径缓存复用已有集成回归通过。修改文件 lint 通过，完整 lint 收尾中。未调用生产模型、未读取或复制密钥。
