...
- 验证：最终完整构建与626项测试通过；新增真实 Worker+D1 覆盖所有者隔离、相关性两次模型核对、来源变动409、核对不通过降级、无接受/阅读写入、问题单独版本、学习精确选择与缺失提示。旧静态测试按参考文献/原地引用展开/预览入口更新。首轮完整 lint 通过，最后一轮检查待收尾。没有本轮浏览器交互/视觉验收或线上真实模型验收，不能把隔离响应当生产科研内容。
