...
test("continuously explores new discovery branches and grows a connected research map", async () => {
  ...
  assert.match(mapRoute, /searchQueries: gapExpanding \? \[gapQuery\] : queries/);
  ...
  assert.match(mapRoute, /action\?: "initialize" \| "hydrate" \| "expand" \| "expand-gap" \| "interpret"/);
  ...
  assert.match(mapRoute, /action\?: "initialize" \| "hydrate" \| "expand" \| "expand-gap" \| "interpret" \| "structure" \| "activity" \| "network" \| "reconcile"/);
  ...
  const researchNetworkStatusStart = client.indexOf("{researchNetworkResponse && (() =>");
  const researchNetworkStatusEnd = client.indexOf("{researchNetworkError &&", researchNetworkStatusStart);
  ...
  assert.match(client, /地图累计 \$\{verifiedCount\} 条已保存数据库关系/);
