...
test("continuously explores new discovery branches and grows a connected research map", async () => {
  ...
  assert.match(client, /setResearchNetworkSimilarityEdges\(payload\.similarityEdges\)/);
  ...
  assert.match(client, /当前是过期缓存，建议刷新/);
  ...
  assert.match(client, /来源短暂限流，已保留当前结果/);
  assert.match(client, /!response\.ok && "status" in data && data\.status === "rate_limited"/);
