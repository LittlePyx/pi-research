...
const access=await checkPilotAccess();
await writeFile(resolve(directory,'access-check.json'),JSON.stringify(access,null,2)+'\n');
if(!access.ready)throw new Error('Experiment access check: '+access.code+'. No capability or model request was sent. See access-check.json.');
