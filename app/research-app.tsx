          ...
          <main className="v2-page v2-learn-page">
            ...
            {activeLearningLoading ? <section className="v2-learning-loading" role="status"><InterfaceIcon name="loading" className="pi-state-mark" /><div><strong>{locale === "zh" ? "正在载入学习路径" : "Loading the learning path"}</strong><i><b /></i></div></section> : activeLearningError && !activeLearningState.path ? <div className="v2-learning-empty error" role="alert"><InterfaceIcon name="warning" className="pi-state-mark" /><h2>{locale === "zh" ? "学习路径载入失败" : "Learning path failed to load"}</h2><p>{activeLearningError}</p><button type="button" onClick={() => setLearningReloadNonce((current) => current + 1)}>{locale === "zh" ? "重新载入" : "Retry"} →</button></div> : activeLearningState.path ? (
              <section className="v2-learning-path">
                ...
                {activeLearningStep && <section id="learning-stage-content" aria-label={locale === "zh" ? "阶段材料" : "Stage materials"} className={`v2-learning-now page ${activeLearningStep.evidenceStatus}`}><header><span>{learningKindLabel(activeLearningStep.kind, locale)}</span><b>{learningEvidenceLabel(activeLearningStep, locale)}</b></header><h3>{locale === "zh" ? activeLearningStep.titleZh : activeLearningStep.titleEn}</h3><p className="v2-learning-goal">{locale === "zh" ? activeLearningStep.goalZh : activeLearningStep.goalEn}</p><LearningNextTask step={activeLearningStep} locale={locale} renderResource={(resource) => <LearningResourceList resources={[resource]} locale={locale} openingId={openingLearningResourceId} onOpen={(item) => void openLearningResource(item)} signals={learningResourceSignals} supplementary />} />{activeLearningStep.resources.length ? <details><summary>{locale === "zh" ? `本阶段全部材料（${activeLearningStep.resources.length}）` : `All stage papers (${activeLearningStep.resources.length})`}</summary><LearningResourceList resources={activeLearningStep.resources} locale={locale} openingId={openingLearningResourceId} onOpen={(resource) => void openLearningResource(resource)} signals={learningResourceSignals} /></details> : <div className="v2-learning-evidence-gap"><strong>{locale === "zh" ? "本阶段材料待补充" : "This stage needs more material"}</strong><p>{locale === "zh" ? "可先查看本阶段目标与补充阅读，合适的材料通过评审后会加入这里。" : "Review the stage goal and supplementary reading. Suitable papers will appear after review."}</p></div>}{Boolean(activeLearningStep.supplementaryResources?.length) && <details><summary>{locale === "zh" ? "补充阅读（不计入本阶段）" : "Supplementary reading (not stage evidence)"}</summary><LearningResourceList resources={activeLearningStep.supplementaryResources || []} locale={locale} openingId={openingLearningResourceId} onOpen={(resource) => void openLearningResource(resource)} signals={learningResourceSignals} supplementary /></details>}<LearningStageGuidance step={activeLearningStep} locale={locale} /><footer><span>{learningTime(activeLearningStep.estimatedMinutes, locale)}</span><button type="button" disabled={Boolean(learningAction) || !canChangeLearningStep(activeLearningStep, currentLearningStep)} onClick={() => void updateLearningStep(activeLearningStep)}>{learningAction === activeLearningStep.id ? "…" : activeLearningStep.status === "completed" ? (locale === "zh" ? "恢复" : "Restore") : (locale === "zh" ? "完成本阶段" : "Complete stage")}</button></footer></section>}
                </div></div>
              </section>
