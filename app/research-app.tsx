...
export default function ResearchApp({ user }: { user: User }) {
  ...
  const scanResearchRouteSignal = async (thread: ResearchTrack, origin: "gap" | "problem") => {
    ...
    if (mapAction || mapBuildTrackId || mapIntelligenceTrackId || !hasQuery
      || activeSpace.id.startsWith("space-") || activeSpace.id.startsWith("local-")) return;
    ...
    try {
      ...
      setToast(origin === "problem"
        ? locale === "zh"
          ? (data.reviewQueuedCount ? `围绕研究问题发现的 ${data.reviewQueuedCount} 篇候选已进入共享质量队列` : "本轮没有新的研究问题候选，检索位置已保存")
          : (data.reviewQueuedCount ? `${data.reviewQueuedCount} research-problem candidates entered the shared quality queue` : "No new research-problem candidate entered the queue; the search position was saved")
        : locale === "zh"
                  ...
                  <section className="v2-route-groups">
                    ...
                    {(["core", "support", "explore"] as ResearchDirectionRole[]).map((role) => researchTracksByRole[role].length ? <section className={`v2-route-group ${role}`} key={role}>
                      ...
                      <div>{researchTracksByRole[role].map((thread) => <article className={thread.buildStatus} key={thread.id}>
                        <header><span className={`v2-direction-heat ${thread.heatLevel}`} title={directionHeatTitle(thread, locale)}><i />{directionHeatLabel(thread.heatLevel, locale)}</span><small>{researchTrackBuildSummary(thread, locale)}</small></header>
              ...
              {visibleLibraryPapers.map((paper) => (
                <article className={"v2-library-paper " + paper.userState} key={paper.id}>
                  <button className="v2-library-paper-main" type="button" onClick={() => openMonitorPaper(paper)}>
                    <div className="v2-library-paper-flags"><span className={"v2-history-state " + paper.userState}>{paper.userState === "unseen" ? t.unseen : paper.userState === "accepted" ? t.accepted : paper.userState === "dismissed" ? t.ignored : paper.userState === "snoozed" ? t.snoozed : t.seenPending}</span><span className={`v2-tier-badge ${paper.qualityStage === "recommended" ? paper.recommendationTier || "browse" : paper.qualityStage === "reviewing" ? "reserve" : "browse"}`}>{paper.qualityStage === "recommended" ? recommendationTierLabel(paper.recommendationTier || "browse", locale) : paper.qualityStage === "reviewing" ? recommendationAuditPhaseLabel(paper, locale) : archiveQualityStagePresentation(paper.qualityStage, locale).label}</span><span>{readingStatusLabel(paper.readingStatus || "unread", locale)}</span>{paper.qualityStage === "recommended" && <span>{t.qualityScore} {displayQualityScore(paper.qualityScore)}</span>}<PaperDiscoverySourceBadge paper={paper} locale={locale} /><RecommendationVerificationBadge paper={paper} locale={locale} /><RouteDiscoveryBadge paper={paper} locale={locale} /></div>
