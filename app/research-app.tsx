...
function ResearchProblemWorkbench({
  ...
  if (loading && !state) return <section className="v2-route-workspace-panel v2-route-panel-empty v2-problem-loading" role="tabpanel"><span>π</span><div><strong>{locale === "zh" ? "正在读取研究问题" : "Loading the research problem"}</strong><p>{locale === "zh" ? "核对问题、假设和最新证据。" : "Checking the problem, hypotheses, and latest evidence."}</p></div></section>;
  if (!state?.problem) return <section className="v2-route-workspace-panel v2-route-panel-empty v2-problem-empty" role="tabpanel"><span>◎</span><div><strong>{locale === "zh" ? "建立研究问题" : "Define a research problem"}</strong><p>{state?.evidence.canDraft ? (locale === "zh" ? "依据当前证据生成问题、范围、成功标准和可检验假设；确认后生效。" : "Draft a question, scope, success criteria, and testable hypotheses from current evidence; active after confirmation.") : (locale === "zh" ? "先完成跨论文综合。" : "Build the cross-paper synthesis first.")}</p>{error && <em>{error}</em>}</div><button type="button" disabled={Boolean(action) || !state?.evidence.canDraft} onClick={onDraft}>{action === "draft" ? (locale === "zh" ? "正在起草…" : "Drafting…") : (locale === "zh" ? "生成草稿" : "Draft problem")} →</button></section>;
    ...
    <div className="v2-app">
      <aside className={"v2-sidebar " + (mobileNav ? "open" : "")}>
        ...
        <div className="v2-sidebar-bottom">
          <a className="v2-demo-entry" href="/demo"><span>◌</span><strong>{locale === "zh" ? "演示空间" : "Demo workspace"}</strong><b>↗</b></a>
      ...
      <div className="v2-main">
        ...
        {view === "learn" && (
          <main className="v2-page v2-learn-page">
            ...
            {activeLearningLoading ? <section className="v2-learning-loading" role="status"><span>π</span><div><strong>{locale === "zh" ? "正在生成学习路径" : "Building the learning path"}</strong><p>{locale === "zh" ? "核对方向与可用论文。" : "Checking the direction and available papers."}</p><i><b /></i></div></section> : activeLearningError && !activeLearningState.path ? <div className="v2-learning-empty error" role="alert"><span>!</span><h2>{locale === "zh" ? "学习路径载入失败" : "Learning path failed to load"}</h2><p>{activeLearningError}</p><button type="button" onClick={() => setLearningReloadNonce((current) => current + 1)}>{locale === "zh" ? "重新载入" : "Retry"} →</button></div> : activeLearningState.path ? (
        ...
        {view === "memory" && (
          <main className="v2-page">
            ...
            <section className="v2-import-memory-card">
              <div><span>π</span><div><p className="v2-kicker">{t.importResearch}</p><h2>{confirmedProfile ? (locale === "zh" ? confirmedProfile.primaryDirectionZh : confirmedProfile.primaryDirectionEn) : (locale === "zh" ? "建立你的研究画像" : "Build your research profile")}</h2></div></div>
        ...
        <div className="v2-modal" role="dialog" aria-modal="true" aria-label={t.importResearch}>
          ...
          <div className="v2-import-modal">
            ...
            {!importDraft ? <div className="v2-import-body">
              ...
              {analyzingImport && <div className="v2-import-progress" role="status" aria-live="polite"><span>π</span><div><strong>{t.analyzingMaterials}</strong><small>{locale === "zh" ? "正在整理研究方向、已有知识与开放问题。" : "Organizing research directions, existing knowledge, and open questions."}</small><i><b /></i></div></div>}
