...
function CitationFlowWorkbench({
  ...
  >
    ...
    <strong><MathText>{item.node.paper.title}</MathText></strong>
        ...
        return <article className="pi-citation-link" key={edge.id}>
          <div className="pi-citation-link-papers">
            <button className="pi-citation-link-paper" type="button" aria-pressed={selectedPaperId === prior.paper.id} onClick={() => { onSelect(prior.paper.id); requestAnimationFrame(() => focusWorkspaceSection("citation-focus-paper")); }}>
              ...
              <strong><MathText>{prior.paper.title}</MathText></strong>
            </button>
            ...
            <button className="pi-citation-link-paper" type="button" aria-pressed={selectedPaperId === later.paper.id} onClick={() => { onSelect(later.paper.id); requestAnimationFrame(() => focusWorkspaceSection("citation-focus-paper")); }}>
              ...
              <strong><MathText>{later.paper.title}</MathText></strong>
            </button>
          </div>
...
function ReadingOrderWorkbench({
  ...
  const renderResource = (resource: LearningResource) => {
    ...
    const content = <><small>{node ? researchPaperYear(node.paper) : resource.publishedAt?.slice(0, 4) || "—"} · {resource.venue}</small><strong><MathText>{resource.title}</MathText></strong><em className="v2-learning-resource-signals">{learningResourceSignals(resource, locale).map((signal) => <i key={signal}>{signal}</i>)}</em></>;
...
function ResearchProblemWorkbench({
  ...
  if (!active || !state?.problem) return <section className="v2-route-workspace-panel v2-research-problem pi-problem-reading" role="tabpanel">
    ...
    <section className="pi-problem-section"><h3>{locale === "zh" ? "已有材料说明了什么" : "What the materials establish"}</h3>
      {synthesis && synthesis.status === "ready" && !synthesis.stale && synthesis.statements.some(item => item.sources.length > 0 && item.sources.every(source => source.evidenceQuote)) ? synthesis.statements.filter(item => item.sources.length > 0 && item.sources.every(source => source.evidenceQuote)).slice(0, 3).map(item => <article key={item.id}><h4><MathText>{locale === "zh" ? item.titleZh : item.titleEn}</MathText></h4><p><MathText>{locale === "zh" ? item.textZh : item.textEn}</MathText></p><ResearchStatementTrace statementIds={[item.id]} synthesis={synthesis} locale={locale} /></article>) : <p>{locale === "zh" ? "现有材料还没有形成可引用的跨论文判断。先核对相关论文的摘要与适用条件。" : "The materials do not yet support a traceable cross-paper finding. Check abstracts and conditions first."}</p>}
    </section>
    ...
    <div className="v2-app pi-workspace">
      ...
      <aside className={"v2-sidebar " + (mobileNav ? "open" : "")} data-pi-dialog={mobileNav ? "mobile" : undefined} role={mobileNav ? "dialog" : undefined} aria-modal={mobileNav ? true : undefined} aria-label={locale === "zh" ? "功能导航" : "Workspace navigation"} tabIndex={-1}>
        ...
        <div className="v2-sidebar-bottom">
          ...
          <button className={`v2-openai-state ${modelConnectionState}`} type="button" onClick={() => { setMobileNav(false); setModelSettingsOpen(true); }} aria-label={locale === "zh" ? "打开 AI 模型设置" : "Open AI model settings"}><i /><span><strong><MathText>{modelConnectionCopy.title}</MathText></strong><small>{modelConnectionCopy.detail}</small></span><b>›</b></button>
                ...
                <div className="v2-daily-brief-list">
                  {visibleBriefEntries.map(({ paper, briefIndex }, index) => {
                    ...
                    return <details key={paper?.id || `${index}:${signal || readingAction}`}>
                      <summary><span>{String(index + 1).padStart(2, "0")}</span><div>{paper && <div className="v2-daily-paper-flags"><i className={`v2-tier-badge ${paper.recommendationTier || "browse"}`}>{recommendationTierLabel(paper.recommendationTier || "browse", locale)}</i><PaperFreshnessBadge paper={paper} locale={locale} /><PaperDiscoverySourceBadge paper={paper} locale={locale} /></div>}<h3><MathText>{paper?.title || (locale === "zh" ? `第 ${index + 1} 篇入选论文` : `Selected paper ${index + 1}`)}</MathText></h3>{paper && <><p className="v2-daily-paper-authors"><span>{paper.authors || (locale === "zh" ? "作者信息未提供" : "Authors unavailable")}</span></p><div className="v2-daily-paper-publication"><span>{formatPaperDate(paper.publishedAt, locale)}</span><span>{paper.venue || (locale === "zh" ? "来源待核对" : "Source pending")}</span><span>{paper.citationCount || 0} {locale === "zh" ? "被引" : "citations"}</span><span>{paper.readMinutes || 15} {locale === "zh" ? "分钟" : "min"}</span></div></>}</div><b aria-hidden="true">＋</b></summary>
                ...
                {researchMapMode === "directions" ? <>
                  ...
                  <div className={`v2-paper-network-stage ${paperNetworkMode === "similarity" ? "discovery-mode" : "analysis-mode"} ${showNetworkPaperDrawer ? "has-drawer" : ""}`}>
                    ...
                    {showNetworkPaperDrawer && selectedNetworkNode && <aside className="v2-paper-network-drawer" aria-label={locale === "zh" ? "论文详情" : "Paper details"}>
                      ...
                      <h2><MathText>{selectedNetworkNode.paper.title}</MathText></h2>
