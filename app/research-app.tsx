...
function ResearchLeadDecisionPanel({
  ...
  return <section className="v2-research-decision-panel" aria-labelledby={`research-decision-${track.id}`}>
    ...
    <div className="v2-research-decision-grid">
      ...
      <article className={`action ${shouldScanGap ? "actionable" : ""}`}><small>{locale === "zh" ? "下一步行动" : "NEXT ACTION"}</small><h3>{nextAction}</h3><p>{activeAction ? (locale === "zh" ? "来自当前研究问题的已保存行动项。" : "Saved from the current research problem.") : shouldScanGap ? (locale === "zh" ? "Pi 已形成定向检索；候选先进入共享质量队列，通过后才会出现在今日。" : "Pi has a targeted query ready. Candidates enter the shared quality queue and reach Today only after passing review.") : (locale === "zh" ? "进入对应工作区继续，不会自动改写正式路线。" : "Continue in the relevant workspace; the formal route is never rewritten automatically.")}</p></article>
    </div>
    <footer><div className="v2-research-decision-funnel" aria-label={locale === "zh" ? "路线质量漏斗" : "Route quality funnel"}>{funnel.map((item, index) => <span key={item.labelEn}><small>{locale === "zh" ? item.labelZh : item.labelEn}</small><strong>{item.value}</strong>{index < funnel.length - 1 && <i>→</i>}</span>)}</div><div className="v2-research-decision-actions">{shouldScanGap && actionableGap ? <button type="button" className="primary" disabled={gapScanBlocked} onClick={() => onScanGap(actionableGap.origin)}>{gapScanning ? (locale === "zh" ? "正在补证据…" : "Finding evidence…") : actionableGap.origin === "problem" ? (locale === "zh" ? "立即检索此不确定性" : "Search this uncertainty now") : (locale === "zh" ? "立即补齐关键证据" : "Close the evidence gap now")} →</button> : <button type="button" className="primary" onClick={() => onOpenWorkspace(primaryTab)}>{primaryLabel} →</button>}{(unresolvedToday > 0 || track.pendingEvidenceCount > 0) && <button type="button" onClick={onOpenToday}>{locale === "zh" ? `处理今日待决 ${Math.max(unresolvedToday, track.pendingEvidenceCount)}` : `Review ${Math.max(unresolvedToday, track.pendingEvidenceCount)} in Today`}</button>}<button type="button" onClick={onOpenLearning}>{locale === "zh" ? "生成学习路径" : "Build learning path"}</button></div><p>{locale === "zh" ? "只有通过共享质量评估的论文才会进入今日；确认或完成阅读后，才会成为正式路线证据。" : "Only papers that pass shared quality review enter Today; confirmation or completed reading is required before they become formal route evidence."}</p></footer>
  </section>;
}
