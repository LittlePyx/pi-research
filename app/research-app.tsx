...
export default function ResearchApp({ user }: { user: User }) {
  ...
  const scanResearchRouteGap = async (thread: ResearchTrack) => {
    ...
    try {
      ...
      const data = await response.json() as ResearchMapState & { addedCount?: number; pendingEvidenceCount?: number; error?: string };
      ...
      setToast(locale === "zh"
        ? (data.addedCount ? `已沿证据缺口找到 ${data.addedCount} 篇待确认论文` : "本轮没有找到足够可信的新证据，检索位置已保存")
        : (data.addedCount ? `Found ${data.addedCount} papers awaiting confirmation` : "No sufficiently grounded new evidence in this pass; the search position was saved"));
    } catch {
              ...
              <>
                {researchMapMode === "directions" ? <>
                  ...
                  <section className="v2-route-groups">
                    <header><div><p className="v2-kicker">{locale === "zh" ? "我的研究布局" : "MY RESEARCH PORTFOLIO"}</p><h2>{locale === "zh" ? "主攻、辅助与探索方向" : "Core, supporting, and exploratory directions"}</h2></div><p>{locale === "zh" ? "候选不会自动变成用户已确认证据；Pi 策展代表作会单独标注。" : "Candidates never become user-confirmed evidence automatically; Pi-curated representatives are labeled separately."}</p></header>
