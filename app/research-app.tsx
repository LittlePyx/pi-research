...
import { emptyResearchMapState, researchRouteLearningSignal, researchRouteOperationalStatus, selectResearchRouteAttention, type ResearchDirectionRole, type ResearchMapState, type ResearchPaperEdge, type ResearchRouteAttentionKind, type ResearchRoutePortfolio, type ResearchTrack, type ResearchTrackPaper, type ResearchTrackRole } from "../lib/research-map";
...
function RoutePortfolioOverview({
  ...
  return <section className="v2-route-portfolio" aria-label={locale === "zh" ? "研究路线证据闭环" : "Research route evidence loop"}>
    <header><div><p className="v2-kicker">{locale === "zh" ? "线索 → 路线 → 今日" : "LEADS → ROUTES → TODAY"}</p><h2>{locale === "zh" ? "研究线索如何变成正式路线证据" : "How research leads become formal route evidence"}</h2><p>{locale === "zh" ? "发现不等于推荐，推荐也不等于正式路线证据；每一步都保留来源和历史。" : "Discovery is not recommendation, and recommendation is not formal route evidence. Provenance and history persist at every step."}</p></div><span className={portfolio.degradedRouteCount > 0 ? "degraded" : portfolio.pausedRouteCount > 0 ? "paused" : "healthy"}><i />{status}</span></header>
...
export default function ResearchApp({ user }: { user: User }) {
  ...
  const routeEvidenceGapTrack = useMemo(() => researchMap.tracks.filter((track) => track.monitoringStatus !== "paused").sort((left, right) => {
                  ...
                  <section className="v2-route-groups">
                    ...
                    {(["core", "support", "explore"] as ResearchDirectionRole[]).map((role) => researchTracksByRole[role].length ? <section className={`v2-route-group ${role}`} key={role}>
                      ...
                      <div>{researchTracksByRole[role].map((thread) => <article className={`${thread.buildStatus} ${thread.monitoringStatus}`} key={thread.id}>
                        ...
                        <RoutePipelineFunnel track={thread} locale={locale} />
                        <RouteLearningNote track={thread} locale={locale} />
