        ...
        {view === "today" && (
          <main className="v2-page v2-today pi-editorial-today">
            ...
            {Boolean(additionalTodayPapers.length) && <section className="v2-today-more pi-unread-recommendations">
              <header className="pi-unread-heading"><div><h2>{locale === "zh" ? "推荐阅读" : "Recommended reading"}</h2><p>{demo ? (locale === "zh" ? "延伸阅读样例，未执行推荐评审" : "Further reading samples, not reviewed recommendations") : (locale === "zh" ? "已通过评审，留给你继续阅读" : "Reviewed papers to continue reading")}</p></div><span>{additionalTodayPapers.length} {locale === "zh" ? "篇未读" : "unread"}</span></header>
