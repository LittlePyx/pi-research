...
function ResearchSynthesisWorkbench({
  synthesis, loading, error, locale, onRefresh, onScanGap, onPaper, onMaterials,
}: {
  track: ResearchTrack;
  synthesis: ResearchSynthesis | null;
  loading: boolean;
  ...
  return <section className="v2-route-workspace-panel v2-research-synthesis" role="tabpanel">
    <header><div><p className="v2-kicker">{locale === "zh" ? "跨论文" : "CROSS-PAPER"}</p><h2>{locale === "zh" ? "共识、分歧与缺口" : "Consensus, conflicts, and gaps"}</h2><span className="v2-sr-only">这条路线目前可以怎样判断</span></div><div>{ready && <span>{synthesis!.confidence}%<small>{locale === "zh" ? "综合置信度" : "confidence"}</small></span>}<button type="button" onClick={onRefresh} disabled={loading || !synthesis?.canGenerate}>{loading ? (locale === "zh" ? "正在综合…" : "Synthesizing…") : ready ? (locale === "zh" ? "按最新证据更新" : "Refresh from evidence") : (locale === "zh" ? "生成跨论文综合" : "Build synthesis")}</button></div></header>
    {!ready && !loading && synthesis && <SynthesisPreparation papers={synthesis.preparation || []} available={availablePapers} locale={locale} onPaper={onPaper} onMaterials={onMaterials} />}
    {ready ? <>
