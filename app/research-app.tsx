...
function ResearchSynthesisWorkbench({
  ...
  return <section className="v2-route-workspace-panel v2-research-synthesis" role="tabpanel">
    ...
    </> : loading ? <div className="v2-route-panel-empty v2-synthesis-loading"><InterfaceIcon name="loading" className="pi-state-mark" /><div><strong>{locale === "zh" ? "正在核对论文证据" : "Checking paper evidence"}</strong><p>{locale === "zh" ? "比较结论、适用条件、冲突和方法演进。" : "Comparing conclusions, conditions, conflicts, and method lineage."}</p></div></div> : <div className="v2-route-panel-empty v2-synthesis-empty"><InterfaceIcon name="synthesis" className="pi-state-mark" /><div><strong>{availablePapers < 2 ? (locale === "zh" ? "至少需要两篇已确认、可核验的论文" : "At least two confirmed, grounded papers are needed") : (locale === "zh" ? "尚无跨论文综合" : "No cross-paper synthesis yet")}</strong><p>{availablePapers < 2 ? (locale === "zh" ? `现有可核验证据：${availablePapers} 篇。` : `Grounded evidence available: ${availablePapers} paper(s).`) : (locale === "zh" ? "现有证据可用于生成带来源的综合。" : "The available evidence can support a source-linked synthesis.")}</p>{track.intelligence && <small>{locale === "zh" ? `路线级初步研判：${track.intelligence.assessmentZh}` : `Preliminary route assessment: ${track.intelligence.assessmentEn}`}</small>}{error && <em>{error}</em>}</div>{availablePapers >= 2 ? <button type="button" onClick={onRefresh}>{locale === "zh" ? "开始综合" : "Build synthesis"} →</button> : <button type="button" onClick={onExplain}>{locale === "zh" ? "查看缺口" : "Review gaps"} →</button>}</div>}
  </section>;
}
...
export default function ResearchApp({ user }: { user: User }) {
  ...
  useEffect(() => {
    if (view !== "thread-detail" || !["start", "assessment"].includes(researchRouteTab) || !selectedThread
    ...
    const load = async () => {
      ...
      try {
        ...
        if (researchRouteTab === "start") return;
