...
function ResearchLeadDecisionPanel({
  ...
  return <section className="v2-research-decision-panel" aria-labelledby={`research-decision-${track.id}`}>
    ...
    <div className="v2-research-decision-grid">
      ...
      <article className={`action ${shouldScanGap ? "actionable" : ""}`}><small>{locale === "zh" ? "下一步行动" : "NEXT ACTION"}</small><h3>{nextAction}</h3><p>{activeAction ? (locale === "zh" ? "来自当前研究问题的已保存行动项。" : "Saved from the current research problem.") : shouldScanGap ? (locale === "zh" ? "Pi 已把这条定向检索排入有上限的后台补证；此处也可立即提前执行。候选通过共享质量评估后才会进入今日。" : "Pi has queued this targeted search as a bounded background evidence task; you can also run it immediately here. Candidates reach Today only after shared quality review.") : (locale === "zh" ? "进入对应工作区继续，不会自动改写正式路线。" : "Continue in the relevant workspace; the formal route is never rewritten automatically.")}</p></article>
    </div>
