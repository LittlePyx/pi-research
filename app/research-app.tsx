                  ...
                  <section className="v2-route-groups">
                    ...
                    {(["core", "support", "explore"] as ResearchDirectionRole[]).map((role) => researchTracksByRole[role].length ? <section className={`v2-route-group ${role}`} key={role}>
                      ...
                      <div>{researchTracksByRole[role].map((thread) => {
                        const recoveryActionNeeded = ["queued", "retryable", "empty", "failed", "partial"].includes(thread.buildStatus);
                        return <article className={`${thread.buildStatus} ${thread.monitoringStatus}`} key={thread.id}>
                          ...
                          <footer><button type="button" onClick={() => openThread(thread)}>{routeMaterialState(thread).status === "exploring" ? (locale === "zh" ? "查看待探索线索" : "View exploratory lead") : (locale === "zh" ? "查看已有材料" : "View collected papers")} →</button></footer>
                        </article>;
                      })}</div>
                    </section> : null)}
                  </section>
          ...
          <main className="v2-page v2-detail-page v2-map-detail v2-route-workspace">
            ...
            {selectedThread ? <>
              ...
              {researchRouteTab === "agenda" && <section className="v2-route-workspace-panel v2-route-agenda-panel" role="tabpanel">
                ...
                {selectedThread.intelligence ? <div className="v2-route-agenda-grid"><article><span>01</span><small>{locale === "zh" ? "深入机会" : "DEEPEN"}</small><h3>{locale === "zh" ? selectedThread.intelligence.opportunityZh : selectedThread.intelligence.opportunityEn}</h3><button type="button" onClick={() => void expandResearchTrack(selectedThread)} disabled={Boolean(mapAction || mapBuildTrackId || selectedThread.monitoringStatus === "paused")}>{selectedThread.monitoringStatus === "paused" ? (locale === "zh" ? "恢复路线后深挖" : "Resume route to mine") : (locale === "zh" ? "继续深挖" : "Explore deeper")} →</button></article><article><span>02</span><small>{locale === "zh" ? "持续观察" : "WATCH"}</small><h3>{locale === "zh" ? selectedThread.intelligence.watchSignalZh : selectedThread.intelligence.watchSignalEn}</h3><button type="button" onClick={() => navigate("today")}>{locale === "zh" ? "查看今日" : "Open Today"} →</button></article><article><span>03</span><small>{locale === "zh" ? "补齐基础" : "BUILD KNOWLEDGE"}</small><h3>{locale === "zh" ? "生成这条路线的论文阅读顺序" : "Build a paper reading order for this route"}</h3><button type="button" onClick={() => openRouteLearningPath(selectedThread)}>{locale === "zh" ? "生成学习路径" : "Build learning path"} →</button></article></div> : <div className="v2-route-panel-empty"><InterfaceIcon name="route" className="pi-state-mark" /><div><strong>{locale === "zh" ? "尚无研究计划" : "No research plan yet"}</strong><p>{locale === "zh" ? "先完成方向研判。" : "Build the direction assessment first."}</p></div><button type="button" onClick={() => setResearchRouteTab("assessment")}>{locale === "zh" ? "查看当前研判" : "Open assessment"} →</button></div>}
          ...
          <main className="v2-page v2-learn-page">
            ...
            {activeLearningLoading ? <section className="v2-learning-loading" role="status"><InterfaceIcon name="loading" className="pi-state-mark" /><div><strong>{locale === "zh" ? "正在载入学习路径" : "Loading the learning path"}</strong><i><b /></i></div></section> : activeLearningError && !activeLearningState.path ? <div className="v2-learning-empty error" role="alert"><InterfaceIcon name="warning" className="pi-state-mark" /><h2>{locale === "zh" ? "学习路径载入失败" : "Learning path failed to load"}</h2><p>{activeLearningError}</p><button type="button" onClick={() => setLearningReloadNonce((current) => current + 1)}>{locale === "zh" ? "重新载入" : "Retry"} →</button></div> : activeLearningState.path ? (
              <section className="v2-learning-path">
                {activeLearningState.path.model === "evidence-structure-v1" && <p role="status">{learningPathResultMessage(activeLearningState.path, locale)} <button type="button" disabled={Boolean(learningAction)} onClick={() => void generateLearningPath()}>{locale === "zh" ? "重试规划" : "Retry planning"}</button></p>}
        ...
        {view === "paper-detail" && selectedMonitorPaper && (
          <main className="v2-page v2-paper-detail">
            ...
            <div className="v2-paper-detail-grid">
              <div>
                <PaperReadingContent key={`${activeSpace.id}:${selectedMonitorPaper.id}`} spaceId={activeSpace.id} paperId={selectedMonitorPaper.id} locale={locale} stage={selectedMonitorPaper.qualityStage} reason={selectedMonitorPaper.screeningReason} />
      ...
      {feedbackPrompt && (
        <div className="v2-modal" role="dialog" aria-modal="true" aria-label={locale === "zh" ? "反馈原因" : "Feedback reason"}>
          ...
          <div className="v2-feedback-reason">
            <div className="v2-modal-head"><div><p className="v2-kicker">{locale === "zh" ? "偏好反馈" : "PREFERENCE FEEDBACK"}</p><h2>{feedbackPrompt.kind === "relevant" ? (locale === "zh" ? "为什么适合？" : "Why is this useful?") : (locale === "zh" ? "为什么不适合？" : "Why is this not useful?")}</h2><p>{feedbackPrompt.paper.title}</p></div><button type="button" onClick={() => setFeedbackPrompt(null)}>×</button></div>
      ...
      {importOpen && (
        <div className="v2-modal" role="dialog" aria-modal="true" aria-label={t.importResearch}>
          ...
          <div className="v2-import-modal">
            <div className="v2-modal-head"><div><p className="v2-kicker">{defaultSpaceName(activeSpace.name, locale)} · {t.privateSpace}</p><h2>{importDraft ? t.importDraftTitle : t.importResearch}</h2>{importDraft && <p>{t.importDraftNote}</p>}</div><button type="button" disabled={analyzingImport || savingImport} onClick={() => setImportOpen(false)}>×</button></div>
      ...
      {askOpen && (
        <div className="v2-modal v2-ask-modal" role="dialog" aria-modal="true" aria-label={t.askTitle}>
          ...
          <div className="v2-ask-panel">
            ...
            {answer && <div className="v2-answer"><div><InterfaceIcon name="synthesis" className="pi-state-mark" /><p className="v2-kicker">{answerMode === "deepseek" ? t.modelAnswer : t.previewMode}</p><small>{answerMode === "deepseek" ? modelDisplayName(answerModel || connectedModel) + " · Chat API" : t.setupRequired}</small></div><p>{answer}</p><div><i />{t.isolated}</div></div>}
