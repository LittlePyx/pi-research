...
export function RouteStart({ track, synthesis, loading, failed, locale, onEvidence, onMaterials, onSynthesis, onPaperOpen, onCompare, onProblem, onLearn, question }: {
  ...
  return <section className="pi-route-start" aria-label={zh ? "当前材料与判断" : "Current materials and findings"}>
    ...
    <div className="pi-route-overview-columns"><section className="pi-route-material-preview">
    <header><h2>{papers.length ? (zh ? "路线材料" : "Route materials") : (zh ? "这条线索还在收集材料" : "This lead is still collecting material")}</h2><p>{papers.length ? (zh ? `${papers.length} 篇已收录 · 选读与核对原文` : `${papers.length} collected · read and check the sources`) : (zh ? "本空间尚未收录可展示的路线材料。这里暂时是一条待探索线索，不能据此判断学界存在空白。" : "This workspace has no route material to display yet. This is an exploratory lead, not evidence of a gap in the field.")}</p></header>
    {papers.length ? <div className="pi-route-start-papers">{papers.slice(0, 4).map(paper => <article key={paper.id}>
      ...
      <h3><MathText inline>{paper.title}</MathText></h3><p>{[paper.authors, paper.venue].filter(Boolean).join(" · ")}</p>
    ...
    </section><section className="pi-route-findings-preview"><h2>{zh ? "已有材料支持的判断" : "Findings grounded in current material"}</h2>
