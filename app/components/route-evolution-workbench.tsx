...
export function RouteEvolutionWorkbench({
  ...
  return <section className="v2-route-evolution">
    ...
    {proposed ? <article className="v2-route-evolution-proposal">
      ...
      <div className="v2-route-evolution-sources"><section><header><strong>{locale === "zh" ? "已确认论文证据" : "Confirmed paper evidence"}</strong><span>{proposed.sourcePapers.length}</span></header>{proposed.sourcePapers.map((paper) => <div key={paper.paperId}><InterfaceIcon name="check" /><span><strong>{paper.title}</strong><small>{[paper.authors, paper.venue, paper.publishedAt?.slice(0, 4)].filter(Boolean).join(" · ")}</small></span></div>)}</section><section className="pi-synthesis"><header><strong>{locale === "zh" ? "Pi 跨论文综合（推断）" : "Pi cross-paper synthesis (inferred)"}</strong><span>{proposed.sourceStatements.length}</span></header>{proposed.sourceStatements.map((statement) => <div key={statement.statementId}><InterfaceIcon name="synthesis" /><span><strong>{locale === "zh" ? statement.titleZh : statement.titleEn}</strong><small>{locale === "zh" ? statement.textZh : statement.textEn}</small></span></div>)}{!proposed.sourceStatements.length && <p>{locale === "zh" ? "本提案只使用论文证据，没有引用额外综合结论。" : "This proposal relies on paper evidence without additional synthesis statements."}</p>}</section></div>
