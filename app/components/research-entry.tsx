...
export function ResearchEntry({spaceId,locale,demo,refreshKey,scanStatus,blocked,onDiscover,onRead,onRoute,onSaved,onReadingList}: {
 ...
 useEffect(()=>{
  ...
  fetch('/api/research-entry?'+new URLSearchParams({spaceId,goalId:chosen.current}),{signal:abort.signal})
 ...
 return <section className="pi-question-entry" aria-label={zh?'问题与第一批阅读':'Question and first reading'}>
  <header><h2>{zh?'从问题开始阅读':'Read with a question'}</h2>{goal&&!editing&&<button type="button" onClick={()=>{setQuestion('');setSeed('');setEditing(true);setSaved(false)}}>{demo?(zh?'体验自定义问题':'Try your own question'):(zh?'提出另一个问题':'Another question')}</button>}</header>
  ...
  {goal&&!editing&&<>
   ...
   {result!.papers.length?<ol className="pi-first-papers">{result!.papers.map(p=><li key={p.id}>
    ...
    <p><span>{zh?'先看哪里':'Start with'}</span><MathText inline>{(zh?p.focusZh:p.focusEn)||(zh?'先读摘要与主要结论，再查对应假设。':'Read the abstract and main result, then check the assumptions.')}</MathText></p>
   ...
   </li>)}</ol>:<p className="pi-question-empty">{demo?(zh?'当前自定义问题没有预设材料。':'No preset papers for this custom question.'):(zh?'尚无已通过评审、明确关联到这个问题的论文。已有阅读仍在下方；不会用其他材料凑数。':'No reviewed papers are linked to this question yet. Existing reading remains below.')}</p>}
