...
export function LearningResourceList({ resources, locale, openingId, onOpen, signals, supplementary = false }: {
  ...
  return <div className="v2-learning-resources v2-learning-reading-list">
    ...
    {resources.map((resource) => {
      ...
      const content = <span><strong><MathText inline>{resource.title}</MathText></strong><small>{[resource.authors, resource.venue, resource.publishedAt?.slice(0, 4)].filter(Boolean).join(" · ")}</small><em className="v2-learning-resource-signals">{signals(resource, locale).join(" · ")}</em></span>;
