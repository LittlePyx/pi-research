...
type ReadingSummary = { zh: string; en: string; sourceUrl: string; kind: "overview" | "abstract-summary" };
type ReadingResponse = { paper?: { abstractText?: string; analysisSource?: string; readingSummary?: ReadingSummary }; recovery?: { status: string; source_url: string; retry_at: number; related?: { abstractText: string; sourceUrl: string; doi: string } | null } | null };
...
export function PaperReadingContent({ spaceId, paperId, locale, stage, reason, onReviewStage }: {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (cancelled) return;
      setSummary(data.paper?.readingSummary || null); setAbstract(data.paper?.abstractText || ""); setRecovery(data.recovery || null);
      if (data.paper?.analysisSource === "deepseek_screened") onReviewStage?.(paperId, "queued");
      if (!data.paper?.readingSummary && (data.paper?.abstractText || "").trim().length < 400 && data.recovery?.status !== "found" && (!data.recovery?.retry_at || data.recovery.retry_at <= Date.now())) {
        setSearching(true);
        ...
        if (!cancelled) { setSummary(data.paper?.readingSummary || null); setAbstract(data.paper?.abstractText || ""); setRecovery(data.recovery || null);
      if (data.paper?.analysisSource === "deepseek_screened") onReviewStage?.(paperId, "queued"); }
      }
  ...
  return <>
    <section className="pi-paper-abstract"><h2>{summary ? (summary.kind === "overview" ? (zh ? "文献简介" : "Paper overview") : (zh ? "摘要概述" : "Abstract summary")) : abstract && abstract.trim().length < 400 && recovery?.status !== "found" ? (zh ? "已保存的简介 · 待核对" : "Saved description · needs checking") : (zh ? "论文摘要" : "Abstract")}</h2>
      {summary ? <><p><MathText>{zh ? summary.zh : summary.en}</MathText></p><a href={summary.sourceUrl} target="_blank" rel="noreferrer">{zh ? "查看来源" : "View source"} ↗</a></> : abstract ? <p><MathText>{abstract}</MathText></p> : <p className="pi-paper-muted" role="status">{searching || recovery?.status === "searching" ? (zh ? "正在查找可核验摘要…" : "Looking for a verifiable abstract…") : recovery?.status === "source_error" ? (zh ? "摘要来源暂时无法访问，将按退避时间重试。" : "Abstract sources are temporarily unavailable; retries respect the source cooldown.") : recovery?.status === "not_found" ? (zh ? "已核对可访问来源，暂未找到匹配摘要。" : "No matching abstract was found in the accessible sources.") : failed ? (zh ? "摘要暂未载入。" : "The abstract could not be loaded.") : abstract === null ? (zh ? "正在读取已保存摘要…" : "Loading the saved abstract…") : (zh ? "当前记录没有摘要，可通过上方原文入口查看。" : "No abstract is saved. Use the source link above to read the paper.")}</p>}
