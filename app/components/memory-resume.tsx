...
export function MemoryResume({ spaceId, routes, locale, onCompare }: { spaceId: string; routes: Route[]; locale: "zh" | "en"; onCompare: (id: string) => void }) {
  ...
  return <section className="pi-memory-resume">
    <header><h2>{zh ? "接着核对上次的问题" : "Resume your comparison"}</h2>{routes.length > 1 && <label>{zh ? "研究路线" : "Route"}<select value={id} onChange={e=>setSelected(e.target.value)}>{routes.map(r=><option key={r.id} value={r.id}>{r.title}</option>)}</select></label>}</header>
    ...
    {!current ? <p role="status">{zh ? "正在找回比较记录…" : "Loading saved comparison…"}</p> : current.failed ? <p role="alert">{zh ? "比较记录暂未载入。" : "Comparison unavailable."}<button onClick={()=>setRetry(n=>n+1)}>{zh ? "重试" : "Retry"}</button></p> : artifact ? <>
      <h3>{zh ? "还需要解决" : "Still unresolved"}</h3><p>{artifact.value.unresolved || (zh ? "上次没有留下待解决问题，可以回到比较记录继续核对。" : "No open question was saved. Revisit the comparison to continue.")}</p>
    ...
    </> : <p>{zh ? "这条路线还没有保存的比较记录。先从下方笔记继续，或在研究页建立比较。" : "No saved comparison on this route. Continue from a note below or start a comparison in Research."}</p>}
  </section>;
}
