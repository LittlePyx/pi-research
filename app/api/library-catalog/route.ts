...
export async function GET(request:Request) {
  ...
  const from=`FROM monitored_papers p LEFT JOIN paper_insights i ON i.paper_id=p.id AND i.space_id=p.space_id LEFT JOIN research_route_library m ON m.paper_id=p.id AND m.space_id=p.space_id AND m.track_id=? LEFT JOIN library_graph_checks g ON g.paper_id=p.id AND g.space_id=p.space_id WHERE ${where.sql}`;
