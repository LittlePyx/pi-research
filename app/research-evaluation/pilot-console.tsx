...
export default function PilotConsole({cases,experimentHash}:{cases:Case[];experimentHash:string}) {
 const token=useRef('');
 ...
 return <main className="pi-pilot">
  ...
  <section aria-label="执行设置"><h2>本次执行</h2>
  ...
  <section aria-label="运行结果"><div className="pi-pilot-heading"><h2>运行记录</h2>{records.length>0&&<button onClick={download}>下载原始记录</button>}</div>
