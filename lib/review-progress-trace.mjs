...
export function traceReviewOutcome(input, emit = emitDefault) {
  ...
  const finalAudit = report.revised || report.audit || report.initial || null;
  const core = ['summary', 'problem', 'method', 'contribution'];
  const grounded = new Set((Array.isArray(finalAudit?.claimChecks) ? finalAudit.claimChecks : [])
    .filter(check => check?.grounded === true).map(check => check.field));
  safeEmit(emit, {
    ...
    finalAuditVerdict: ['verified', 'revise', 'insufficient'].includes(finalAudit?.verdict) ? finalAudit.verdict : null,
    finalAuditCoverage: finalAudit ? Math.max(0, Math.min(100, Number(finalAudit.coverageScore) || 0)) : null,
    uncoveredCoreFields: finalAudit ? core.filter(field => !grounded.has(field) || !finalAudit.supportedFields?.includes(field)) : [],
    unsupportedCoreFields: core.filter(field => finalAudit?.unsupportedFields?.includes(field)),
    finalAuditIssueCount: ['unsupportedFields', 'overstatements', 'contradictionRisks'].reduce((count, key) =>
      count + (Array.isArray(finalAudit?.[key]) ? finalAudit[key].length : 0), 0),
