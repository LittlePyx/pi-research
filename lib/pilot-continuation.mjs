...
export async function continuePilot({cases,experimentHash,sourceCommit,executionSourceCommit,token,endpoint='/api/personalization-pilot',fetchImpl=fetch,onRecord=()=>{},onProgress=()=>{}}) {
 const records=[];
 for(const task of cases)for(const variant of task.order) {
  onProgress(task,variant,records.length+1);
  const response=await fetchImpl(endpoint,{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+token},body:JSON.stringify({caseId:task.id,variant,sourceCommit,executionSourceCommit}),signal:AbortSignal.timeout(75_000)});
