...
export function parsePilotResponse(data,task) {
 if(data.choices?.[0]?.finish_reason!=='stop')throw new Error('incomplete_model_response');
 let payload;try{payload=JSON.parse(data.choices[0].message.content);}catch{throw new Error('invalid_model_json');}
 const rows=payload.recommendations;
 if(!Array.isArray(rows)||rows.length>5)throw new Error('invalid_ranking');
 const ids=new Set();
 for(const row of rows) {
  const paper=task.candidates.find(p=>p.id===row?.id);
  if(!paper||ids.has(row.id)||typeof row.reason!=='string'||!row.reason.trim()||row.reason.length>1200||typeof row.quote!=='string'||row.quote.length<20||row.quote.length>400||!paper.abstract.includes(row.quote))throw new Error('ungrounded_or_duplicate_ranking');
  ids.add(row.id);
 }
 return rows.map(({id,reason,quote})=>({id,reason,quote}));
}

export async function executePilotRun(experiment,task,variant,{apiKey,sourceCommit,fetchImpl=fetch,now=()=>Date.now()}) {
 ...
 const record={caseId:task.id,variant,contextId:task.contextId,poolHash:task.poolHash,experimentHash:experiment.experimentHash,
  model:experiment.model,promptVersion:experiment.version,sourceCommit,observedAt:new Date(started).toISOString(),
 ...
 try {
  ...
  const allowed=['incomplete_model_response','invalid_model_json','invalid_ranking','ungrounded_or_duplicate_ranking'];
  record.errorCode=allowed.includes(error.message)?error.message:error.name==='TimeoutError'?'timeout':'transport_or_parse_error';
...
export function summarizePilot(experiment,records) {
 ...
 return {experimentHash:experiment.experimentHash,status:complete?'rankings_recorded_awaiting_blind_review':'incomplete',plannedRuns:rows.length,completedRuns:rows.filter(r=>r.status==='completed').length,
