...
export function buildReliabilityProgram(input) {
  ...
  const evaluated = succeeded.filter((job) => job.completionState !== "analysis_unavailable");
  const analysisUnavailableRuns = succeeded.filter((job) => job.completionState === "analysis_unavailable").length;
  ...
  const zeroRecommendationRuns = evaluated.filter((job) => job.recommendedCount === 0).length;
  let consecutiveZeroRecommendationRuns = 0;
  for (const job of ordered) {
    if (job.status !== "ready") continue;
    if (job.completionState === "analysis_unavailable") continue;
  ...
  const stageReliability = Object.values((input.stageEvents || []).reduce((stages, event) => {
    const stage = event.stage || "unknown";
    const current = stages[stage] || { stage, attempts: 0, failures: 0, durations: [] };
    current.attempts += 1;
    if (event.outcome === "failed" || event.outcome === "degraded") current.failures += 1;
    if (Number.isFinite(event.durationMs) && event.durationMs >= 0) current.durations.push(event.durationMs);
    ...
    p50DurationMs: percentile(stage.durations, 0.5),
    p95DurationMs: percentile(stage.durations, 0.95),
  })).sort((left, right) => right.p95DurationMs - left.p95DurationMs || left.stage.localeCompare(right.stage));
  const alerts = [];
  ...
  if (analysisUnavailableRuns) alerts.push({
    code: "analysis_unavailable",
    severity: analysisUnavailableRuns >= 2 ? "critical" : "warning",
    titleZh: "部分扫描未完成论文解读",
    titleEn: "Some scans could not complete paper review",
    detailZh: `近 14 天有 ${analysisUnavailableRuns} 次扫描保存了候选，但模型未能完成任何深度解读；这些运行不应计为“零推荐”。`,
    detailEn: `${analysisUnavailableRuns} scan(s) saved candidates but completed no deep review; these runs must not be counted as zero-recommendation quality outcomes.`,
  });
  ...
  return {
    ...
    targets: {
      ...
      deepAnalysisAvailabilityRate: 95,
    },
    actual: {
      ...
      analysisUnavailableRuns,
      analysisAvailabilityRate: succeeded.length ? Math.round((succeeded.length - analysisUnavailableRuns) / succeeded.length * 100) : 0,
      staleActiveJobs,
      sourceFailureCount,
    },
    evaluation: {
