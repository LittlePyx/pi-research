...
export const ROUTE_SCHEDULER_ORDINAL_SQL = `SELECT COUNT(*) AS count FROM monitor_scheduler_ticks WHERE trigger_source != 'visit_backstop'`;
export function scheduledRouteTaskOrder(count) {
  const lanes = ['routeIntelligence', 'routeEvolution', 'routeRetry'];
  const start = Math.max(0, Number(count || 1) - 1) % lanes.length;
  return [...lanes.slice(start), ...lanes.slice(0, start)];
}
...
// Include the persisted request-deadline timeout, not arbitrary model errors:
// credential/balance failures must not become automatic paid retry loops.
...
export const SCHEDULED_MONITOR_ERROR_SPACE_SQL = `SELECT s.id, s.owner_user_id
 ...
 WHERE s.owner_user_id LIKE 'anonymous:%' AND r.automation_paused_at IS NULL
  AND r.status = 'error' AND r.error IN ('stale_scheduler_recovery', 'The operation was aborted due to timeout')
