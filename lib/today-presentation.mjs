...
// Presentation only: retain original brief indexes for their saved guidance.
export function remainingBriefEntries(entries, featuredIds = []) {
  const featured = new Set(featuredIds);
  return entries.filter(({paper}) => !featured.has(paper.id));
}

// Only explicit reading states establish prior reading; opening/saving does not.
export function isReadReference(paper) {
  return ['read', 'mastered', 'cited'].includes(paper?.readingStatus);
}

// A resume action follows an explicit reading state, never a click or a save.
export function resumeReading(papers) {
  return papers.filter(paper => paper.readingStatus === 'reading')
    .sort((a, b) => (Date.parse(b.openedAt) || 0) - (Date.parse(a.openedAt) || 0))[0] || null;
}

export function readingReferences(papers, limit = 6) {
  const seen = new Set();
  return papers.filter(paper => {
    if (seen.has(paper.id) || !isReadReference(paper)
      || (paper.qualityStage !== 'recommended' && !paper.demo)) return false;
    seen.add(paper.id);return true;
  }).slice(0, limit);
}
