  ...
  if (path === '/api/feedback' && method === 'POST' && ['save','relevant','not_relevant','later'].includes(body.kind)) {
    ...
    if (kind === 'relevant' || kind === 'not_relevant') {
      ...
      if (value && reason) data.monitor.preferenceSignals.push({id:signalId,layer:'explicit',kind:reason.kind,
        labelZh:`${kind === 'relevant' ? '偏好' : reason.kind === 'mastery' ? '已掌握' : '排除'}：${paper.title}`,
        labelEn:`${kind === 'relevant' ? 'Prefer' : reason.kind === 'mastery' ? 'Mastered' : 'Exclude'}: ${paper.title}`,
        evidence:`本次演示反馈：${reason.zh}${typeof body.note === 'string' && body.note.trim() ? ' · '+body.note.trim().slice(0,500) : ''}`,
        confidence:96,effectiveConfidence:92,sourceType:'paper_feedback',sourcePaperId:paper.id,observedAt:new Date().toISOString(),expiresAt:null});
