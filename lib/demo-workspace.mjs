...
export async function demoResponse(input, init = {}) {
  ...
  if (path === '/api/research-memory' && method === 'GET') {
    ...
    const items = papers.filter(p => p.readingNote && `${p.title} ${p.readingNote}`.includes(q)).map(p => ({paperId:p.id,title:p.title,venue:p.venue,note:p.readingNote,updatedAt:DEMO_NOTES[p.id]?.date || '2026-09-20',status:'pending',takeawayZh:'',takeawayEn:'',methodsZh:[],methodsEn:[],questionsZh:[],questionsEn:[]}));
