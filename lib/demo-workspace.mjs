  ...
  if (path === '/api/feedback' && method === 'POST' && ['save','relevant','not_relevant','later'].includes(body.kind)) {
    ...
    if (kind === 'relevant' || kind === 'not_relevant') {
      ...
      if (value && reason) data.monitor.preferenceSignals.push({id:signalId,layer:'explicit',kind:reason.kind,
        ...
        confidence:96,effectiveConfidence:92,sourceType:'paper_feedback',observedAt:new Date().toISOString(),expiresAt:null});
    ...
    return response({ok:true,state:paper.userState,routeEvidence:null,effect:{zh:reason && value ? '已将本次演示反馈加入明确偏好；刷新后重置，不执行真实推荐。' : '已保存本次演示操作；刷新后重置。',en:'Saved in this demo session; resets on refresh. No real recommendation is run.'}});
  }
