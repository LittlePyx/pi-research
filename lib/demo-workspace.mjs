...
export function learningFixture(spaceId = spaces[0].id) {
  ...
  const papers = samples.map((sample, index) => ({
    ...
    summaryZh: sample.note, summaryEn: 'UI fixture, not a real quality assessment.',
    whyReadZh: sample.note, whyReadEn: 'Test material presentation', priorityVenue: false,
    analysisSource: 'demo', screeningReason: '', userState: 'seen', showCount: 0, saved: false, feedback: null,
    firstShownAt: null, lastShownAt: null, openedAt: null, snoozedUntil: null, readingStatus: 'unread', readingNote: index === 0 ? '示例笔记：核对论文的模型假设、结论范围与适用条件。' : '',
  ...
  const steps = kinds.map((kind, index) => ({
    ...
    goalZh: ['建立问题定义与基本语言', '理解关键证明工具', '比较代表性结果', '识别尚需核对的条件', '整理一个可验证的问题'][index], goalEn: 'QA stage goal',
    whyZh: '对照文献导读与原文，区分已知结果和待核对的问题。', whyEn: 'QA specific contribution', readFocusZh: '先记录模型假设，再比较结论。', readFocusEn: 'QA specific assumptions',
    checkpointZh: '能否说清这篇材料的适用范围？', checkpointEn: 'QA specific decision',
    estimatedMinutes: 30, status: index === 0 ? 'active' : 'pending', position: index,
    resources: index < papers.length ? [{ ...papers[index], id: `monitor:${papers[index].id}`, canonicalId: papers[index].id,
  ...
  return {
    learning: { suggestedTarget: space.description, availablePaperCount: papers.length, waitingQualityCount: 0, model: 'fixture',
      path: { id: `${spaceId}-path`, target: space.description, targetTrackId: "demo-route", titleZh: `${space.name} 学习路径`, titleEn: `${space.name} learning path`,
    ...
    monitor: { preferences:{profileKey:'demo',profileNameZh:space.description,profileNameEn:'Demo sources',priorityVenues:[...new Set(samples.map(p=>p.venue))],explorationMode:'balanced',trackedAuthors:[],userModified:false}, status: 'ready', lastRunAt: null, nextRunAt: null, newCount: 0, scannedCount: 2, knownCount: 2, error: null,
      cadenceHours: 24, source: '演示样例', horizons: ['years'], papers: papers.slice(0, 3), historyPapers: papers, throttled: true, dailyBrief: { date: '2026-09-20', isCurrent: false, status: 'ready', headlineZh: '从经典问题走向核心方法', headlineEn: 'From foundations to methods', overviewZh: '固定的演示简报，展示阅读流程；不是实时推荐或正式评审。', overviewEn: 'Fixed demo brief, not a live recommendation.', signalsZh: [], signalsEn: [], readingPlanZh: [], readingPlanEn: [], watchlistZh: [], watchlistEn: [], paperIds: papers.slice(0, 3).map(p => p.id), metrics: {}, model: 'demo', error: null, updatedAt: '2026-09-20' } },
  };
}
...
function demoMap(data) {
  ...
  track.id = 'demo-route';
  track.titleZh = data.learning.path.titleZh.replace(' 学习路径', '研究路线');
  ...
  track.confirmedEvidenceCount = papers.length;
  map.generated = true;
  return map;
}
...
export async function demoResponse(input, init = {}) {
  ...
  if (path === '/api/paper-reading' && method === 'GET') return response({paper:{abstractText:''}});
