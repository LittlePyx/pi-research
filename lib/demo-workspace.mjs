import { FEEDBACK_REASONS } from "./preference-memory.ts";
import { DEMO_READING_CONTEXT } from "./demo-reading-context.mjs";
...
export function learningFixture(spaceId = spaces[0].id) {
  ...
  const papers = samples.map((sample) => ({
    ...
    whyReadZh: DEMO_READING_CONTEXT[sample.id]?.reasonZh || sample.note, whyReadEn: DEMO_READING_CONTEXT[sample.id]?.reasonEn || 'Use the original source to check assumptions and scope.', demoReadingContext:DEMO_READING_CONTEXT[sample.id] || null, priorityVenue: false,
...
export async function demoResponse(input, init = {}) {
  ...
  const papers = data.monitor.historyPapers;
  // A preset connection is usable only while all of its source notes still match.
  const matchesNote = paperId => papers.find(p => p.id === paperId)?.readingNote.trim() === DEMO_NOTES[paperId]?.note.trim();
  for (const item of papers) {
    const context = DEMO_READING_CONTEXT[item.id];
    const valid = context && matchesNote(context.paperId);
    item.demoReadingContext = valid ? context : null;
    item.whyReadZh = valid ? context.reasonZh : item.summaryZh;
    item.whyReadEn = valid ? context.reasonEn : 'Use the original source to check assumptions and scope.';
  }
  data.monitor.preferenceSignals = data.monitor.preferenceSignals.filter(signal => !signal.sourcePaperIds || signal.sourcePaperIds.every(matchesNote));
  const paperId = url.searchParams.get('paperId') || body.paperId;
  const paper = papers.find(p => p.id === paperId);
  const path = url.pathname;
  if (path === '/api/research-entry' && ['GET','POST'].includes(method)) {
    const math = id === 'demo-mathematics';
    if (method === 'POST') {
      if (body.action === 'reset') delete data.entryQuestion;
      else {
        const question = typeof body.question === 'string' ? body.question.trim() : '';
        if (question.length < 10 || question.length > 520 || (body.seed || '').length > 300) return response({error:'Invalid question'},400);
        data.entryQuestion = question;
      }
    }
    const ids = math ? ['eldan-thin-shell','lee-vempala-localization','chen-kls'] : ['finite-blocklength','lossy-finite','conditional-dispersion'];
    const goal = {id:math?'demo-question-kls':'demo-question-coding',trackId:data.entryQuestion?'':'demo-route',question:data.entryQuestion || (math?'随机局部化如何连接薄壳估计与 KLS 谱隙界？':'有限码长下，信道错误概率与有损压缩的超额失真约束如何影响二阶项？'),status:'active',monitoringStatus:'active'};
    const selected = data.entryQuestion ? [] : ids.map(id=>papers.find(p=>p.id===id)).filter(p=>p&&!['read','mastered','cited'].includes(p.readingStatus)&&!['dismissed','snoozed'].includes(p.userState));
    return response({goal,papers:selected.map(p=>({id:p.id,title:p.title,whyZh:p.whyReadZh,whyEn:p.whyReadEn,
      focusZh:math?'先找主要定理中的条件与维数依赖，再核对所用局部化步骤。':'先看问题定义与概率约束，再定位二阶项的表述。',
      focusEn:math?'Start with theorem assumptions and dimension dependence, then inspect the localization step.':'Start with the problem definition and probability criterion, then locate the second-order term.',
      checkZh:math?'记录与前一篇相比改变了哪些假设、哪些界，原文依据在哪里。':'区分源、信道与边信息设置，记录哪些结论可比较，哪些不可直接类比。',
      checkEn:'Record the assumptions, differences from the previous paper, and exact source locations.'}))});
  }
  ...
  if (path === '/api/preference-signals' && method === 'PATCH') {
    ...
    if (body.active && signal.sourcePaperIds && !signal.sourcePaperIds.every(matchesNote)) return response({error:'Source note changed'},409);
  ...
  if (path === '/api/feedback' && method === 'POST' && ['save','relevant','not_relevant','later'].includes(body.kind)) {
    if (!paper) return response({error:'Paper not found'},404);
    if (typeof body.value !== 'boolean') return response({error:'Boolean value required'},400);
    const reason = Object.hasOwn(FEEDBACK_REASONS,body.reasonCode || '') ? FEEDBACK_REASONS[body.reasonCode] : null;
    if (body.reasonCode && (!reason || (body.kind !== 'relevant' && body.kind !== 'not_relevant') || (body.kind === 'relevant') !== (reason.polarity === 'positive'))) return response({error:'Invalid reason'},400);
    const kind = body.kind, value = body.value;
    if (kind === 'save') paper.saved = value;
    if (kind === 'relevant' || kind === 'not_relevant') {
      paper.feedback = value ? kind : null;
      if (kind === 'not_relevant' && value) paper.saved = false;
      const signalId = `${id}:feedback:${paper.id}`;
      data.monitor.preferenceSignals = data.monitor.preferenceSignals.filter(s => s.id !== signalId);
      if (value && reason) data.monitor.preferenceSignals.push({id:signalId,layer:'explicit',kind:reason.kind,
        labelZh:`${kind === 'relevant' ? '偏好' : reason.kind === 'mastery' ? '已掌握' : reason.kind === 'exclusion' ? '排除' : reason.zh}：${paper.title}`,
        labelEn:`${kind === 'relevant' ? 'Prefer' : reason.kind === 'mastery' ? 'Mastered' : reason.kind === 'exclusion' ? 'Exclude' : reason.en}: ${paper.title}`,
        evidence:`本次演示反馈：${reason.zh}${typeof body.note === 'string' && body.note.trim() ? ' · '+body.note.trim().slice(0,500) : ''}`,
        confidence:96,effectiveConfidence:92,sourceType:'paper_feedback',reasonCode:body.reasonCode,sourcePaperId:paper.id,observedAt:new Date().toISOString(),expiresAt:null});
      if (value && body.reasonCode === 'duplicate_known') paper.readingStatus = 'mastered';
    }
    paper.userState = !value ? (paper.feedback === 'relevant' || paper.saved ? 'accepted' : 'seen') : kind === 'not_relevant' ? 'dismissed' : kind === 'later' ? 'snoozed' : 'accepted';
    paper.snoozedUntil = kind === 'later' && value ? new Date(Date.now()+3*86400000).toISOString() : null;
    data.monitor.papers = papers.filter(p=>p.qualityStage === 'recommended' && p.userState === 'seen');
    data.monitor.historyCounts = {all:papers.length,inbox:papers.filter(p=>p.userState==='seen'||p.userState==='unseen').length,accepted:papers.filter(p=>p.userState==='accepted').length,dismissed:papers.filter(p=>p.userState==='dismissed').length};
    return response({ok:true,state:paper.userState,routeEvidence:null,effect:{zh:reason && value ? '已记录“'+reason.zh+'”，可在研究偏向查看；刷新后重置，不执行真实推荐。' : !value ? '已撤回本次反馈，阅读状态与笔记保留；刷新后重置。' : '已保存本次演示操作；刷新后重置。',en:'Saved in this demo session; resets on refresh. No real recommendation is run.'}});
  }
