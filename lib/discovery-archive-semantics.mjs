...
export function archiveQualityStagePresentation(stage, locale = "en") {
  const zh = locale === "zh";
  if (stage === "awaiting_evidence") return { label: zh ? "摘要待补全" : "Abstract needed", kicker: zh ? "材料不足" : "MATERIALS INCOMPLETE", note: zh ? "摘要证据不足，暂无法完成评审；这不是论文质量不合格。" : "Abstract evidence is missing. Review cannot be completed; this is not a negative quality judgment." };
