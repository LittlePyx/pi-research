...
export async function handleServerPilot(request,{database,apiKey,experiment,accessHash,expiresAt,executeRun=executePilotRun,fetchImpl=fetch,now=Date.now()}) {
 ...
 if(!body||Object.keys(body).some(k=>!['caseId','variant','sourceCommit','executionSourceCommit'].includes(k)))return reply({error:'fixed_experiment_only'},400);
 ...
 if(body.executionSourceCommit!==undefined&&!/^[0-9a-f]{40}$/.test(body.executionSourceCommit))return reply({error:'invalid_execution_revision'},400);
 ...
 const record=await executeRun(experiment,task,body.variant,{apiKey,sourceCommit:body.sourceCommit,executionSourceCommit:body.executionSourceCommit||body.sourceCommit,fetchImpl});
