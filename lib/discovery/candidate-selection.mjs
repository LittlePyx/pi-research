...
export const PROVENANCE_FAIRNESS_AGE_MS = 30 * 60 * 1000;

/**
 * Give old route-origin candidates a bounded opportunity before another
 * high-scoring branch can keep renewing the head of the shared queue. The
 * learning and explicit evidence-gap lanes win before other route work, with
 * the oldest candidate first inside each lane. This only changes evaluation
 * order and never marks a paper reviewed or recommended.
 *
 * @template T
 * @param {T[]} rankedItems
 * @param {{
 ...
 *  lane: (item: T) => string,
 *  firstSeenAt: (item: T) => string | null | undefined,
 * }} options
 * @returns {T[]}
 */
export function selectAgedProvenanceFairCandidates(rankedItems, options) {
  const limit = Math.max(0, Math.floor(options.limit));
  if (!limit) return [];
  const now = Number.isFinite(options.now) ? Number(options.now) : Date.now();
  const ageMs = Math.max(0, Number(options.ageMs ?? PROVENANCE_FAIRNESS_AGE_MS));
  const unique = new Map();
  for (const item of rankedItems) {
    const id = options.canonicalId(item);
    if (id && !unique.has(id)) unique.set(id, item);
  }
  const aged = [...unique.values()].map((item, rank) => {
    const firstSeen = Date.parse(options.firstSeenAt(item) || "");
    return { item, rank, firstSeen, lane: options.lane(item) };
  }).filter((entry) => entry.lane && Number.isFinite(entry.firstSeen) && now - entry.firstSeen >= ageMs)
    .sort((left, right) => {
      const priority = (lane) => lane === "learning" ? 3 : lane === "gap" ? 2 : lane === "route" ? 1 : 0;
      return priority(right.lane) - priority(left.lane) || left.firstSeen - right.firstSeen || left.rank - right.rank;
    });
  const protectedItems = selectBalancedByGroup(aged, (entry) => entry.lane, limit).map((entry) => entry.item);
  return protectedItems.slice(0, limit);
}
...
export function selectBudgetedDeepReviewCandidates(items, options = {}) {
  ...
  // Aged learning, gap, and route candidates get a bounded review opportunity
  // inside this same budget. They still had to clear the unchanged fast-screen
  // gate before reaching this allocator and must clear the final gate later.
  const agedRouteTarget = Math.min(routeTarget, Math.max(0, limit - selected.length));
  for (const item of selectAgedProvenanceFairCandidates(
    ranked.filter((candidate) => candidate.isRouteOrigin),
    {
      limit: agedRouteTarget,
  ...
  const routeRanked = ranked.filter((item) => item.isRouteOrigin).sort((left, right) =>
    Number(right.isCurrentDiscovery) - Number(left.isCurrentDiscovery)
      || Number(right.evidenceReady) - Number(left.evidenceReady)
      || Number(right.score) - Number(left.score));
  const routeCount = () => selected.filter((item) => item.isRouteOrigin).length;
  addBalanced(routeRanked, routeTarget - routeCount(), (item) => item.routeKey || item.directionKey);
