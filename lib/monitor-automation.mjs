...
export function monitorAutomationPauseReason(input) {
  ...
  if (input.pendingRecommendations >= limits.pendingRecommendations) return "pending_backlog";
...
export function monitorAutomationPauseCopy(reason, pendingRecommendations = 0) {
  if (reason === "pending_backlog") return {
    zh: `已有 ${pendingRecommendations} 篇推荐等待处理，自动扫描已待机。处理一部分或手动扫描后恢复。`,
    en: `${pendingRecommendations} recommendations await review, so automatic scanning is on standby. Process some or scan manually to resume.`,
  };
