...
export function monitorScanCompletionLabel(monitor, locale) {
  if (monitor?.qualityQueue) {
    if (monitor.qualityQueue.pendingCount > 0) return locale === "zh" ? "检索已结束 · 候选评审尚未完成" : "Discovery ended · candidate review remains";
    if (monitor.qualityQueue.awaitingAbstractCount > 0) return locale === "zh" ? "检索已结束 · 部分材料待补摘要" : "Discovery ended · some abstracts are missing";
...
/** Status is based on the live, eligible queue, never a capped paper sample or an old job's totals. */
export function monitorQualityReviewStatus(monitor, now = Date.now()) {
  const queue = monitor?.qualityQueue;
  if (!queue || !(queue.pendingCount + queue.awaitingAbstractCount)) return null;
  const retryAt = monitor.status === "error" ? monitor.scanJob?.nextRetryAt || monitor.nextRunAt : monitor.nextRunAt;
  const nextAt = queue.pendingCount > 0 ? retryAt : queue.abstractRetryAt ? new Date(queue.abstractRetryAt).toISOString() : null;
  const due = timestamp(nextAt);
  let state = "scheduled";
  if (monitor.automation?.paused) state = "paused";
  else if (!["idle", "ready", "error"].includes(monitor.status) && timestamp(monitor.leaseExpiresAt) > now) state = "active";
  else if (monitor.status === "error") state = "retry";
  else if (!queue.pendingCount) state = "evidence";
  else if (!Number.isFinite(due)) state = "unscheduled";
  else if (now - due > 25 * 60 * 1000) state = "overdue";
  return { ...queue, state, nextAt: nextAt || null,
    firstReviewCount: Math.max(0, queue.pendingCount - queue.verificationCount - queue.retryCount) };
}
