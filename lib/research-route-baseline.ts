export const RESEARCH_ROUTE_BASELINE_MODEL = "system-baseline";

export const RESEARCH_ROUTE_BASELINE_SQL = `
...
SELECT 'route-baseline:' || track.id, track.space_id, track.id, 1, 'confirmed',
 ...
 '[]', '[]', '[]', '[]', 0, '${RESEARCH_ROUTE_BASELINE_MODEL}',
 CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
FROM research_tracks track
WHERE track.space_id = ?
 AND NOT EXISTS (
  SELECT 1 FROM research_route_revisions revision WHERE revision.track_id = track.id
 )`;

export async function ensureResearchRouteBaselines(database: D1Database, spaceId: string) {
  if (!spaceId) return;
  await database.prepare(RESEARCH_ROUTE_BASELINE_SQL).bind(spaceId).run();
}
